...
for p in range(128, 256):
    ...
    if str(bin(p)[2:]).rjust(8, '0')[4:] == p2[:4] == MASK:
        if e[:4] == e2[4:]:
            print(f"m = {str(bin(p)[2:]).rjust(8, '0')}, c = {e}, "
                    f"m* = {p2}, c* = {e2}")
            k1, k2, kk1, kk2 = find_sub_key(str(bin(p)[2:]).rjust(8, '0'), p2, e, e2)
            print(f"First two rounds k1 = {k1}, k2 = {k2}")
            print(f"Last two rounds k1 = {kk1}, k2 = {kk2}")
            print("")
            for k in k1:
                for kk in k2:
                    try_key.append(k+kk)
...
for item, amount in keys.items():
    print("Key: {} \tfrequency of occurrence - {}".format(item, amount))